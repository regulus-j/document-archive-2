    ...
    <div class="max-w-lg mx-auto">
        <div class="bg-white rounded-xl shadow-xl border border-blue-100 overflow-hidden">
            <div class="bg-white p-6 border-b border-blue-200">
                <h1 class="text-xl font-bold text-gray-800">Edit Document Category</h1>
            </div>
            <div class="p-6">
                <form action="{{ route('document-categories.update', $documentCategory) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="category" class="block text-sm font-medium text-gray-700 mb-1">Category Name</label>
                        <input type="text" id="category" name="category"
                            value="{{ old('category', $documentCategory->category) }}"
