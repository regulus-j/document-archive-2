                                ...
                                @foreach($document->versions as $version)
                                    <div class="flex items-start gap-3 p-3 rounded-lg border border-slate-100 hover:bg-slate-50 transition group">
                                        ...
                                        <div class="flex items-center gap-1">
                                            ...
                                            @if((int)$version->uploaded_by === (int)auth()->id() || auth()->user()->hasRole('super-admin') || auth()->user()->hasRole('company-admin'))
                                                <form action="{{ route('documents.versionDelete', [$document->id, $version->id]) }}" method="POST" class="inline" onsubmit="return confirm('Delete version {{ $version->version_number }}? This cannot be undone.')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="p-1.5 rounded-lg hover:bg-red-100/50 text-slate-400 hover:text-red-600 transition" title="Delete v{{ $version->version_number }}">
                                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                                    </button>
                                                </form>
                                            @endif
    ...
    function renderXlsxInModal(url, container) {
        ...
        fetch(url)
            ...
            .catch(function(err) {
                ...
                } else if ((err.message || '').toLowerCase().indexOf('workbook') !== -1) {
        ...
        fetch(previewUrl)
            ...
            .catch(function(err) {
                if ((err.message || '').toLowerCase().indexOf('workbook') !== -1) {
                    xlsxEl.innerHTML = '<div class="text-center py-4"><p class="text-xs text-slate-500 mb-3">This file is not a spreadsheet. Trying alternate preview...</p><iframe src="' + previewUrl + '" class="w-full border border-slate-200 rounded-lg bg-white" style="min-height:65vh;"></iframe></div>';
                } else {
                    xlsxEl.innerHTML = '<div class="text-center py-8"><p class="text-sm text-red-500">Failed to render spreadsheet.</p></div>';
                }
            });
    } else {
        hideLoading();
