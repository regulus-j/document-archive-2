        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-indigo-200/80 overflow-hidden">
                <div class="bg-gradient-to-r from-indigo-50 to-white p-4 border-b border-indigo-200/60 flex items-center justify-between">
                    ...
                    <span class="text-xs font-medium text-indigo-600 bg-indigo-100 px-2 py-0.5 rounded-full">{{ $document->versions->count() + 1 }} version(s)</span>
                </div>
                <div class="p-4">
                    {{-- Current version --}}
                    <div class="flex items-center gap-3 p-2.5 rounded-lg border border-emerald-200 bg-emerald-50/50 mb-2">
                        <div class="flex-shrink-0 w-8 h-8 bg-emerald-500 rounded-full flex items-center justify-center text-white text-[10px] font-bold">Now</div>
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-1.5">
                                <p class="text-sm font-medium text-slate-800">Current Version</p>
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium bg-emerald-100 text-emerald-700">Active</span>
                            </div>
                            <p class="text-xs text-slate-500">{{ $document->updated_at->format('M d, Y g:ia') }}</p>
                        </div>
                        @php
                            $currentExt = strtolower(pathinfo($document->path, PATHINFO_EXTENSION));
                        @endphp
                        <button onclick="openDocumentViewer('{{ route('documents.previewCurrent', $document->id) }}', '{{ $currentExt }}')"
                    ...
                    <div class="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                        @foreach($document->versions as $version)
                            <div class="flex items-center gap-3 p-2.5 rounded-lg border border-slate-100 hover:bg-slate-50 transition group">
                                <div class="flex-shrink-0 w-8 h-8 bg-slate-300 rounded-full flex items-center justify-center text-white text-[10px] font-bold">v{{ $version->version_number }}</div>
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="text-xs text-slate-500">
                                        ...
                                        @if($version->file_size)
                                            &middot; {{ $version->formatted_size }}
                                        @endif
                                    </p>
                                    @if($version->change_notes)
                                        <p class="text-xs text-slate-400 italic mt-0.5">&ldquo;{{ $version->change_notes }}&rdquo;</p>
                                    @endif
                                </div>
                                <div class="flex items-center gap-1">
                                    @php
                                        $versionExt = strtolower(pathinfo($version->original_filename, PATHINFO_EXTENSION));
                                    @endphp
                                    <button onclick="openDocumentViewer('{{ route('documents.versionPreview', [$document->id, $version->id]) }}', '{{ $versionExt }}')"
