...
<div class="bg-slate-100 min-h-screen py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-bold text-slate-900 mb-6">Your Managed Companies</h1>

        <div class="bg-white shadow-xl rounded-lg overflow-hidden">
            <div class="p-6 border-b border-slate-200">
                <div class="flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
                    <a href="{{ route('companies.create') }}"
                        class="w-full sm:w-auto px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-300">
                        Add New Company
                    </a>
                        ...
                        @forelse ($companies as $company)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                    ...
                                    <div class="text-xs text-slate-400">{{ $company->registered_name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                    <div>{{ $company->company_email }}</div>
                                    <div class="text-xs text-slate-400">{{ $company->company_phone }}</div>
                                </td>
