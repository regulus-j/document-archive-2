            ...
            <div class="bg-white p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-500">View and manage document information</p>
                    </div>
                </div>
                <a href="{{ route('admin.document-management.documents') }}"
                    class="inline-flex items-center px-4 py-2 border border-indigo-200 text-sm font-medium rounded-lg shadow-sm text-indigo-700 bg-indigo-50 hover:bg-indigo-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
        ...
        <div class="bg-white rounded-xl shadow-xl overflow-hidden border border-indigo-100 mb-6">
            <div class="bg-white p-6 border-b border-indigo-200">
                <div class="flex items-center">
                    ...
                    <h2 class="text-lg font-semibold text-slate-800">{{ $document->title }}</h2>
                </div>
            </div>
            <div class="p-6">
                <div class="bg-indigo-50 rounded-lg p-6 border border-indigo-100 mb-6">
                    <div class="flex items-center mb-4">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-slate-600">Uploaded by <span
                                    class="font-medium text-slate-900">{{ $document->user->name }}</span></p>
                            <p class="text-sm text-slate-500">{{ $document->created_at->format('M d, Y') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        @if($document->description)
                            <div>
                                ...
                                <p class="text-slate-900">{{ $document->description }}</p>
                            </div>
                        @endif
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                @empty
                                    <span class="text-slate-500">No categories assigned</span>
                                @endforelse
                            </div>
                        </div>
                        ...
                        <div>
                            <h3 class="text-sm font-medium text-slate-500 mb-1">Attachments</h3>
                                ...
                                <ul class="space-y-2">
                                    @foreach($document->attachments as $attach)
                                        <li class="flex items-center">
                                            ...
                                            <a href="{{ Storage::disk('public')->url($attach->path) }}" target="_blank"
        ...
        <div class="bg-white rounded-xl shadow-xl overflow-hidden border border-indigo-100">
            <div class="bg-white p-6 border-b border-indigo-200">
                <div class="flex items-center">
                    ...
                    <h2 class="text-lg font-semibold text-slate-800">Audit Logs</h2>
                </div>
            </div>
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center text-sm text-slate-500">
                                    <div class="flex flex-col items-center justify-center py-6">
                                        ...
                                        <p class="text-slate-500 text-base">No audit logs available.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
